...
from foia_hub.scripts import scrape_contacts


class ScrapeContactsTests(TestCase):
    def setUp(self):
        self.html = """
            <h1>An Agency</h1>
    ...
    def test_agency_description(self):
        ...
        description = scrape_contacts.agency_description(doc)
    ...
    def test_find_emails(self):
        ...
        emails = scrape_contacts.find_emails(lines, ps)
        self.assertEqual(["a@b.com", "c@d.com", "e@f.info", "g@h.io"],
                         emails)

        emails = scrape_contacts.find_emails(lines[:1], ps[:1])
        self.assertEqual([], emails)

        lines = ["Off hand reference to email"]
        ps = [BeautifulSoup("<p>Off hand reference to email</p>")]
        self.assertRaises(Exception, scrape_contacts.find_emails, lines, ps)
    ...
    def test_find_bold_fields(self):
        ...
        ps = [BeautifulSoup(p) for p in ps]
        results = list(scrape_contacts.find_bold_fields(ps))
